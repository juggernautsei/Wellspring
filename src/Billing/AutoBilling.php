...
class AutoBilling
{
    ...
    public function billingEntries($dx, $cpt, $units)
    {
        /**
        ...
        if($howmanydx == 1 && $howmanycpts == 1) {
            $cpt = str_replace(",,", "", $cpt);
            $dx = str_replace(",,", "", $dx);
            $this->generateBilling($dx, $cpt, $units);
        } else {
            $this->genMultiCode();
        }
    }

    public function generateBilling($dx, $cpt, $units)
    {
        /**
       ...
       //$enc = $this->getEventEncounter($pid, $event_date);
        ...
        if (empty($hasBilling['encounter'])) {
            ...
            if (!empty($code)) {
                    $codetype = "CPT4";
                    $this->insertCPTBilling($event_date, $codetype, $code, $this->pid, $provider, $this->userid, $enc, $text, $modifier, $units, $fees, $icd10);
            }
     ...
     * @return void
     */
    private function insertCPTBilling($event_date, $codetype, $code, $pid, $provider, $userid, $enc, $text, $modifier, $units, $fees, $icd10): void
    {
        $sql = "REPLACE INTO billing SET " .
            ...
            "pricelevel = 'standard'";
             ...
             $justify = "ICD10|$icd10:";
              sqlStatementThrowException($sql, [$event_date, $codetype, $code, $pid, $provider, $userid, $enc, $text, $modifier, $units, $fees, $justify]);
